...
# def recon_loss(recon_x, x, mu, logvar, variational_beta, imgSize, channel,  bsz=64):
#     MSE = F.mse_loss(recon_x,x)
#     KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
#     # KLD += -0.5 * torch.sum(1 + logvar2 - mu2.pow(2) - logvar2.exp())
#     # Normalise by same number of elements as in reconstruction
...
# def recon_loss_x_2(recon_x, x, mu2, logvar2, variational_beta, imgSize, channel,  bsz=64):
#     MSE = F.mse_loss(recon_x,x)
#     KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
#     KLD += -0.5 * torch.sum(1 + logvar2 - mu2.pow(2) - logvar2.exp())
#     # Normalise by same number of elements as in reconstruction
